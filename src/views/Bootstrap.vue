	...
	<div id="app">
		<div class="container">
			...
			<div style="padding: 1rem;">
				<!-- <div v-for="brand in ['success', 'info', 'warning', 'danger']" class="alert" :key="brand" :class="'alert-' + brand">
						<strong>Well done!</strong> You successfully read this {{brand}} alert message.
					</div> -->
				<b-alert v-for="brand in ['success', 'info', 'warning', 'danger']" :key="brand" :variant="brand" show dismissible>
					<strong>Well done sir!</strong> You successfully read this {{brand}} alert message.
				</b-alert>
			</div>
			<h2>Breadcrumb</h2>
			<div style="padding: 1rem;">
				...
				<b-breadcrumb :items="[{text: 'Home', href: '#'}, {text: 'Library', href: '#'}, {text: 'Data', active: true }]"></b-breadcrumb>
			</div>
			<!-- Buttons-->
			<h2>Buttons</h2>
			<div style="padding: 1rem;">
				<div v-for="type in ['', 'outline']" :key="type">
					<span v-for="brand in ['primary', 'secondary', 'success', 'info', 'warning', 'danger']" :key="brand">
						<b-button :variant="(type !== '' ? type + '-' : '') + brand" style="margin-bottom: 1rem; margin-right: 1rem;">{{brand}} {{type}}</b-button>
